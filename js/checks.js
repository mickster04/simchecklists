function checklist_item_cross() {
  ...
  if (this.getBoundingClientRect().top > screen.height * 0.65) {
    window.scroll({
      top: this.getBoundingClientRect().top - 10 + window.scrollY,
      behavior: "smooth",
    });
  }
  ...
  for (i = 0; i < itemsList.length; i++) {
    if (this.innerHTML == itemsList[i].innerHTML) {
      itemsList[i + 1].classList.add("highlight");
      break;
    }
  }
}
  ...
  if (child.length > 0) {
    for (i = 0; i < child.length; i++) {
      if (child[i].classList.contains("items")) {
        ...
        if (child[i].classList.contains("highlight")) {
          child[i].classList.remove("highlight");
        }
      }
      if (i == child.length - 1) {
        window.scroll({
          top: child[i].getBoundingClientRect().top - 10 + window.scrollY,
          behavior: "smooth",
        });
      }
    }
...
async function checklist_load_masterlist() {
  ...
  for (i = 0; i < masterlist.checklist.length; i++) {
    checklist_display_title(
      "checklists",
      masterlist.checklist[i].file,
      masterlist.checklist[i].name
    );
  }
}
...
function checklist_display_title(element_id, file, name) {
  ...
  element.innerHTML +=
    "- <a href='checklist.html?l=" + file + "'>" + name + "</a><br>";
}
...
function checklist_process() {
  ...
  for (var i = 0; i < itemsList.length; i++) {
    ...
    itemsList[i].addEventListener("touchstart", function (event) {
  ...
  for (var i = 0; i < titlesList.length; i++) {
    titlesList[i].addEventListener("dblclick", checklist_subcheckcross, false);
  }
}
  ...
  for (i = 0; i < array.length; i++) {
    ...
    if (!line_array[1]) {
      ...
      if (line_array[0].substring(0, 2) == "**") {
        ...
        contentHTML +=
          "<div class='comment purple highlight'>" + comment + "</div>";
      }
    } else {
