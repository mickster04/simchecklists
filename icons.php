<div class="icons">
	<div><a href="index.php"><?PHP $theme->theme_icon('back'); ?></a></div>
	<div class="icao_open"><?PHP $theme->theme_icon('icao'); ?></div>
	<div class="canvas_open"><?PHP $theme->theme_icon('canvas'); ?></div>
	<div class="writer_open"><?PHP $theme->theme_icon('writer'); ?></div>
	<div class="chrono_open"><?PHP $theme->theme_icon('timer'); ?></div>
	<div onclick="location.reload(true);"><?PHP $theme->theme_icon('refresh'); ?></div>
</div>